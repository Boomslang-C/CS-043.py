    ...
    if path == '/register' and un and pw:
        ...
        if user:
            ...
            if 'HTTP_COOKIE' in environ:
                ...
                cookies.load(environ['HTTP_COOKIE'])
                if 'score' in cookies:  # values are set on line 114 so first time users might not have any scores
                    # taken from project 5 instructions
            ...
            if 'factor1' in params and 'factor2' in params and 'answer' in params:
                ...
                if f1*f2 == ans:
                    correct += 1
                    page += '<p style="background-color: lightgreen">Correct, {} x {} = {}</p>'.format(f1, f2, ans)
                else:
                    page += '<p style="background-color: red">Wrong, {} x {} = {}, not {}</p>'.format(f1, f2, f1 * f2, ans)
                    wrong += 1
            ...
            answer = [f1*f2, random.randrange(100) + 1, random.randrange(100) + 1, random.randrange(100) + 1]
            random.shuffle(answer)

            hyperlink = '<a href="/account?username={}&amp;password={}&amp;factor1={}&amp;factor2={}&amp;answer={}">{}</a><br>'

            page += ('<h2>A:' + hyperlink.format(un, pw, f1, f2, answer[0], answer[0]) + '</h2>' +
                     '<h2>B:' + hyperlink.format(un, pw, f1, f2, answer[1], answer[1]) + '</h2>' +
                     '<h2>C:' + hyperlink.format(un, pw, f1, f2, answer[2], answer[2]) + '</h2>' +
                     '<h2>D:' + hyperlink.format(un, pw, f1, f2, answer[3], answer[3]))
        ...
        page = '''<!DOCTYPE html>
                        <html>
                            <head>
                                <title>Multiply with Score</title>
                            </head>
                            <body>
                                <h1>Login</h1>
                                <form action="/login">
